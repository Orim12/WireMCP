    ...
    async (args) => {
        try {
            ...
            await fs
                ...
                .catch((err) =>
                    console.error(
                        `Failed to delete ${tempPcap}: ${err.message}`,
                    ),
                );
...
server.tool(
    ...
    async (args) => {
        try {
            ...
            const tempPcap = "temp_capture.pcap";
            console.error(
                `Capturing summary stats on ${interface} for ${duration}s`,
            );
...
server.tool(
    ...
    async (args) => {
        try {
            ...
            const tempPcap = "temp_capture.pcap";
            console.error(
                `Capturing conversations on ${interface} for ${duration}s`,
            );
...
server.tool(
    ...
    async (args) => {
        try {
            ...
            const tempPcap = "temp_capture.pcap";
            console.error(
                `Capturing traffic on ${interface} for ${duration}s to check threats`,
            );
            ...
            console.error(
                `Captured ${ips.length} unique IPs: ${ips.join(", ")}`,
            );
            ...
            try {
                ...
                console.error(
                    `URLhaus response status: ${response.status}, length: ${response.data.length} chars`,
                );
                console.error(
                    `URLhaus raw data (first 200 chars): ${response.data.slice(0, 200)}`,
                );
                ...
                console.error(
                    `Sample URLhaus IPs: ${urlhausData.slice(0, 5).join(", ") || "None"}`,
                );
                urlhausThreats = ips.filter((ip) => urlhausData.includes(ip));
                console.error(
                    `Checked IPs against URLhaus: ${urlhausThreats.length} threats found - ${urlhausThreats.join(", ") || "None"}`,
                );
            } catch (e) {
    ...
    async (args) => {
        try {
            ...
            try {
                ...
                console.error(
                    `IP ${ip} checked against URLhaus: ${isThreat ? "Threat found" : "No threat found"}`,
                );
            } catch (e) {
                console.error(`Failed to fetch URLhaus data: ${e.message}`);
                urlhausData = [];
    ...
    async (args) => {
        try {
            ...
            const urls = packets
                ...
                .map(
                    (p) =>
                        `http://${p._source.layers["http.host"][0]}${p._source.layers["http.request.uri"][0]}`,
                );
            console.error(
                `Found ${urls.length} URLs: ${urls.join(", ") || "None"}`,
            );

            const protocols = [
                ...new Set(
                    packets.map(
                        (p) => p._source?.layers["frame.protocols"]?.[0],
                    ),
                ),
            ].filter((p) => p);
...
server.tool(
    ...
    async (args) => {
        try {
            ...
            console.error(
                `Found ${credentials.plaintext.length} plaintext and ${credentials.encrypted.length} encrypted credentials`,
            );
...
server.prompt(
    "capture_packets_prompt",
    {
        ...
        duration: z
            .number()
            .optional()
            .describe("Duration in seconds to capture"),
    },
